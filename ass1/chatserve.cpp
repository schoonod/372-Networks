...
// CS372
// Project 1
// Description: This program is a server that connects with a client 
// client and exchanges messages until a host closes the connection.
...
int main(int argc, char ** argv){
    ...
    // Run the chat until client or host quits
    while(1){
        receiveMessage(chatSocket);

        // If the chat socket closes
        if (!chatSocket)
            break;
